...
class CommentsController {
    ...
    async getAllComments(req, res, next) {
        try {
            if (req.query.hasOwnProperty('type') && req.query.hasOwnProperty('courseID') && req.query.hasOwnProperty('replyID') && req.query.hasOwnProperty('page')) {
                ...
                const countAll = Comment.countDocuments({ course_id: req.query.courseID })
                const itemAll = Comment.find({ course_id: req.query.courseID, commenttable_type: req.query.type, reply_id: req.query.replyID }).sort({ '_id': -1 }).limit(limit).skip(skip)
    ...
    //     try {
    //         const confirmLoveBlog = await reactComment.findOne({ commentID: req.params.id, userID: req.id })
    ...
    //         if (confirmLoveBlog) {
    //             if (req.body.type == confirmLoveBlog.type) {
    //                 const deleteLoveBlog = await reactComment.deleteOne({ _id: confirmLoveBlog._id })
    //                 if (deleteLoveBlog) {
    ...
    //                     const updateBlog = await Comment.updateOne({ commenttable_id: req.params.id }, {
    //                         $set: {
    //                             reactions_count: countAmountOfReact,
    //                             reaction_summary: type
    //                         },
    //                         $pull: { reactions: { _id: confirmLoveBlog._id } },
    //                         new: true
    //                     })
    //                     if (updateBlog) {
    ...
    //                 const updateLoveBlog = await reactComment.updateOne({ _id: confirmLoveBlog._id }, {
    //                     $set: {
    //                         type: req.body.type
    //                     }
    //                 })
    //                 if (updateLoveBlog) {
    //                     const getLoveComment = await reactComment.findOne({ _id: confirmLoveBlog._id })
    //                     const countAmountOfReactTypeOld = await reactComment.countDocuments({ commentID: confirmLoveBlog.commentID, type: confirmLoveBlog.type })
    //                     const countAmountOfReactType = await reactComment.countDocuments({ commentID: req.params.id, type: req.body.type })
    //                     const type = {
    //                         ...dataComment.reaction_summary,
    //                         [req.body.type]: countAmountOfReactType,
    //                         [confirmLoveBlog.type]: countAmountOfReactTypeOld
    //                     }
    //                     if (countAmountOfReactTypeOld <= 0) {
    //                         delete type[confirmLoveBlog.type]
    //                     }
    //                     const updateBlog = await Comment.updateOne({ commenttable_id: req.params.id, 'reactions._id': confirmLoveBlog._id }, {


    //                         $set: {
    //                             reaction_summary: type,
    //                             reactions: getLoveComment
    //                         },
    ...
    //                         const newDataBlog = {
    //                             ..._doc,
    //                             reactionText: textReaction[req.body.type]
    //                         }
    ...
    //             }
    //             const rBlog = await reactComment.create(datas)
    //             if (rBlog) {
    //                 const countAmountOfReact = await reactComment.countDocuments({ commentID: req.params.id })
    //                 const countAmountOfReactType = await reactComment.countDocuments({ commentID: req.params.id, type: req.body.type })
    //                 const type = {
    ...
    //         const confirmLoveBlog = await reactComment.findOne({ commentID: req.params.id, userID: req.id });
    //         const dataComment = await Comment.findOne({ commenttable_id: req.params.id });
    //         console.log(req.body, confirmLoveBlog);

    //         if (confirmLoveBlog) {
    //             await handleExistingLoveBlog(req, res, confirmLoveBlog, dataComment);
    //         } else {
    //             await handleNewLoveBlog(req, res, dataComment);
    //         }
    //     } catch (err) {
    //         console.error(err);
    ...
    // async handleExistingLoveBlog(req, res, confirmLoveBlog, dataComment) {
    //     try {
    //         if (req.body.type == confirmLoveBlog.type) {
    //             await deleteLoveBlogAndUpdateSummary(req, res, confirmLoveBlog, dataComment);
    //         } else {
    //             await updateLoveBlogAndSummary(req, res, confirmLoveBlog, dataComment);
    //         }
    //     } catch (err) {
    //         console.error(err);
    //         return res.json({ success: 0, message: "Error processing existing love blog" });
    //     }
    // }

    // async deleteLoveBlogAndUpdateSummary(req, res, confirmLoveBlog, dataComment) {
    //     const deleteLoveBlog = await reactComment.deleteOne({ _id: confirmLoveBlog._id });
    //     if (deleteLoveBlog) {
    //         const countAmountOfReactType = await reactComment.countDocuments({ commentID: req.params.id, type: req.body.type });
    //         const type = {
    ...
    //             return res.json({ success: 1, data: dataBlog });
    //         } else {
    //             return res.json({ success: 0, message: "Deletion unsuccessful" });
    //         }
    //     }
    // }

    // async updateLoveBlogAndSummary(req, res, confirmLoveBlog, dataComment) {
    //     const updateLoveBlog = await reactComment.updateOne(
    //         { _id: confirmLoveBlog._id },
    //         { $set: { type: req.body.type } }
    //     );
    //     if (updateLoveBlog) {
    //         const getLoveComment = await reactComment.findOne({ _id: confirmLoveBlog._id });
    ...
    //         }
    //         const updateBlog = await Comment.updateOne(
    //             { commenttable_id: req.params.id, 'reactions._id': confirmLoveBlog._id },
    //             {
    //                 $set: {
    //                     reaction_summary: type,
    //                     reactions: getLoveComment
    ...
    //             return res.json({ success: 1, data: dataBlog });
    //         } else {
    //             return res.json({ success: 0, message: "Update unsuccessful" });
    //         }
    //     }
    // }

    // async handleNewLoveBlog(req, res, dataComment) {
    //     const datas = {
    //         userID: req.id,
    //         commentID: req.params.id,
    //         type: req.body.type,
    ...
    //             return res.json({ success: 1, data: dataBlog });
    //         } else {
    //             return res.json({ success: 0, message: "Save unsuccessful" });
    //         }
    //     }
    // }
